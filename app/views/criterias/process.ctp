		...
		if(!empty($document['files'])) {
			...
			if(count($document['files']) === 1){
				...
				if(!is_null($mime_type))
					echo '<a href="../documents/getFile?title='.$document['Document']['name'].'&id='.$document['Document']['id'].'&filename='.$file['Attachfile']['name'].'&mime='.$mime_type.'" style="margin: 0 0 0 10px;">
